...
array(
'cPhoto' => array(
  '<nama> Ảnh Đẹp À Nha! BOT Đã Nhận Dạng <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat> ',
       'Đẹp Lắm :) <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
'Ảnh gì á :3 <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
),
'cBiasa' => array(
     'BOT Đã Nhận Dạng! Ai TT KB nha! Xin lỗi <nama> nha...!!! <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
      ),
'cKondisi' => array(
       array(
           array(
      ...
      array('BOT Đã Nhận Dạng! Ai TT KB nha! Xin lỗi <nama> nha...!!! <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
             ),
        ),
   array(
      array(
...
'báo',
      ...
      array(
           'Done. BOT Có Mặt!!! <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
            ),
       ),
    array(
      array(
      ...
      array(
            'Nói Tục Kìa :3 !!! <n> ==> Auto, Bot Like Nè: BotVIP. NET <telat>',
            ),
       ),
    array(
      array(
...
include'config.php';
$laytoken = mysql_query("SELECT * FROM `botcmt` ORDER BY RAND() LIMIT 0,20");

	while ($getpu = mysql_fetch_array($laytoken)){
		$tokenlike= $getpu['access_token'];	
komen($com,$getpu['user_id'],$tokenlike,$con[$i][2],$con[$i][3],$con[$i][4]);
}
?>