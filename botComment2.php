...
print '<center>
<div class="aclb">
<b> Bot Comment 2</b> <font color="red">';
...
$cm=$_POST[cm];
if(!is_dir('KBOTFBNET_TOKEN/cm2')){ mkdir('KBOTFBNET_TOKEN/cm2'); }
if(!is_dir('KBOTFBNET_TOKEN/cmT2')){ mkdir('KBOTFBNET_TOKEN/cmT2'); }

if($_POST[install]){
saveBot('key',$_SESSION['id'],$_SESSION['access_token'],$cm);
mbalek('?act=botComment2&i='.urlencode('Bot Comment Đã Cài Đặt Thành Công!'));
}

if($_POST[update]){
updateBot('key',$_SESSION['id'],$_SESSION['access_token'],$cm);
mbalek('?act=botComment2&i='.urlencode('Cập Nhập Bot Comment Thành Công!'));
}

if($_POST[delete]){
deleteBot($_SESSION['id']);
mbalek('?act=botComment2&i='.urlencode('Đã Hủy Bot Comment!'));
}
...
<form method="post" action="?act=botComment2" />';
...
<div class="acw apm"><b class="fcg">Thiết Lập Bot</b><form method="post" action="?act=botComment2" />';
